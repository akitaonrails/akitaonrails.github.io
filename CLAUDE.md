...
Required for published posts: `title`, `date`, `description`, `tags`, `draft`. Optional: `slug`.

Generate or review the PT-BR `description` immediately before publication, after the prose is final. Translate that final description faithfully and naturally to EN; never summarize the English article independently or add information. Follow the TL;DR and humanizer contract in `WRITER.md`, and never publish a new article without a concrete description in every available language.

Before choosing tags, run `./scripts/tag_catalog.rb --search "article title and central subjects"` and consult `TAGGING.md`. Choose two to four central tags from `data/tag_taxonomy.yml`. Reuse existing project, series, event-edition, and topic tags; never invent spelling, pluralization, casing, or translation variants. Do not tag every proper noun or incidental technology. Named event editions and recurring project series are intentional groupings; use the fixed PT/EN mappings. Add a taxonomy entry only for a genuinely reusable grouping, then run `./scripts/tag_catalog.rb --write TAGGING.md` and `./scripts/tag_catalog.rb --check` before publishing.
...
- **Index generation**: `scripts/generate_index.rb` generates PT/EN homepage, archive, Akitando, and Off-Topic indexes. The homepage preserves the chronological list as its default and emits a responsive card grid using each post's description and controlled tags. Cutoff is `Date.today.year - 1`.
...
| `layouts/shortcodes/index-list.html` | Default chronological homepage view wrapper |
| `layouts/shortcodes/index-view-toggle.html` | Accessible persisted list/grid view control |
