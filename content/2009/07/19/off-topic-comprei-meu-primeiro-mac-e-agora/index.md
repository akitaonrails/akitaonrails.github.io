...
translationKey: off-topic-comprei-meu-primeiro-mac-e-agora
...
Você é um “switcher”: decidiu dar o grande salto, deixar um pouco de lado o mundo Windows e comprar seu primeiro Mac. A época é boa e os preços estão acessíveis, especialmente se você consegue comprar com alguém de fora. Tenha uma coisa em mente desde já: isso é uma mudança de paradigmas.

A primeira semana ou duas são as piores. Você sempre vai ter aquele pensamento de _“no Windows eu fazia isso mais fácil…”_ ou _“no meu Linux eu fazia isso mais fácil…”_. A tentação de voltar é grande quando você sai da zona de conforto pela primeira vez, como em qualquer mudança. Force-se a aprender os truques novos em vez de recorrer aos velhos.
...
- O Mac OS Leopard (10.5) não vem com o layout de teclado US International que é comum no Windows para teclados não ABNT-2 (que não tem cedilha). O normal do Mac é usar atalhos como ⌥+c para cedilha, ⌥+e para acento agudo ⌥+` para acento grave, ⌥+n para til. Mas se quiser o jeito tradicional baixe o layout US International para Mac a partir do site do brasileiro [Rainer Brokerhoff](http://web.archive.org/web/20090802092433/http://www.brockerhoff.net/usi/) e siga suas instruções. A partir do Snow Leopard (10.6) isso não é mais necessário.
...
- Instalar aplicativos no Mac costuma confundir no começo. Normalmente você baixa um de dois formatos: DMG ou MPKG. O primeiro significa (D)isk i(M)a(G)e e é parecido com um ISO. Quando você dá duplo clique, o DMG se “monta” dentro de /Volumes. O aplicativo já costuma estar lá dentro, e basta arrastar para a pasta “Applications” do seu Mac. Depois você “ejeta” o DMG com o ícone ⏏ ou com ⌘+E. O segundo formato, (M)ac (P)ac(K)a(G)e, é o equivalente ao MSI do Windows: você dá duplo clique e ele roda como um instalador tradicional. Normalmente só programas que precisam instalar muita coisa de sistema usam MPKG, como Microsoft Office e Adobe CS4. Às vezes você baixa um ZIP que contém um DMG ou um MPKG. O importante: se for um DMG sem MPKG dentro, sempre arraste a aplicação para Applications antes de executá-la. O Safari faz um passo extra: baixa o DMG, monta, extrai o aplicativo e apaga o DMG, daí você arrasta a partir da pasta Downloads. Um detalhe técnico: MPKGs vêm com um BOM (Bill Of Materials), que funciona como receita das permissões originais dos arquivos de sistema para scripts que ajustam permissões. O mesmo BOM serve para desinstalar, já que registra tudo que foi instalado.
...
- Toda vez que você baixar um programa da internet, na primeira execução o Mac vai lhe mostrar a URL de onde veio o download e pedir confirmação para abrir. Para ser avisado sempre que você ou algum processo tentar modificar uma configuração de sistema, vá em “System Preferences”, “Security” e habilite “Require password to unlock each System Preferences pane”. O Mac costuma avisar toda vez que algo tenta escalar autorização, e você precisa passar sua senha de administrador para confirmar. Para quem vem do Linux, é parecido com usar “sudo” no Terminal. O Mac é bastante seguro: a arquitetura impede que malwares triviais se apropriem da máquina como acontece no Windows, e isso não tem a ver com market share, já que o usuário precisa explicitamente fazer uma bobagem para ser invadido. Antivírus só serve para garantir que você não repasse a outra pessoa um anexo infectado que recebeu de alguém. A sua máquina sai ilesa, mas você pode acabar encaminhando o vírus por acidente.
...
- O Mac é um Unix BSD. Como tal ele possui um aplicativo de Terminal na pasta de Utilities. Use o Spotlight para acessá-lo mais facilmente. Ele já abre um shell BASH, e a partir daí você pode manipular arquivos e programas de maneira muito simples usando um ambiente POSIX. Se você vem de Linux, vai se sentir em casa.

- Falando em Linux, infelizmente uma deficiência é a falta de um bom gerenciador de pacotes. No Ubuntu/Debian temos o Apt-get, no Redhat/Fedora/CentOS temos o Yum. No Mac não tem nenhum padrão. A recomendação é usar o [HomeBrew](http://web.archive.org/web/20120419165352/http://mxcl.github.com/homebrew/) que é um sistema para gerenciar a instalação e manutenção de projetos open source. Ele não é tão poderoso nem tão robusto quanto um Aptitude num Ubuntu, mas até que faz bem o seu trabalho. Você também vai acabar encontrando outros como o MacPorts e o Fink, mas eles ou estão desatualizados ou dão mais trabalho e, por isso, não recomendo usá-los, veja a próxima dica.
...
- Se tiver um iPad, você pode usá-lo como se fosse um segundo monitor, ligado ao seu Mac via Wi-Fi (precisam estar na mesma rede). Para isso compre o programa [Air Display](http://web.archive.org/web/20120420081551/http://avatron.com/apps/air-display).
...
- Para ripar DVDs e criar mp4 use o freeware [HandBrake](http://handbrake.fr/). Para converter vídeos para iPod, iPhone, use o [iSquint](http://www.macupdate.com/info.php/id/19769). Para criar, editar e converter arquivos de legendas use o [Miyu](http://www.fluffalopefactory.com/miyu/) e o [Submerge](http://www.bitfield.se/submerge/index.html).

- Para apresentações, esqueça o Powerpoint. Vale **muito** a pena comprar o [iWork](http://web.archive.org/web/20110107211503/http://www.apple.com/iwork/download-trial/) para usar o famoso Keynote. O pacote iLife e iWork está todo disponível via App Store, comprem por lá.

![](http://s3.amazonaws.com/akitaonrails/assets/2009/7/20/whatsnew-hero-20090106_original.jpg)

- Para se comunicar, use o [Adium](http://adium.im/) que é um cliente para Jabber, Messenger, Yahoo, AIM, etc, um dos melhores que tem, mas não suporta ainda video chat para Messenger. O clássico [Skype](http://www.skype.com/intl/pt/download/skype/macosx/) também é outra ótima opção. Se gosta de IRC um dos melhores clientes é o [Colloquy](http://web.archive.org/web/20090717071516/http://colloquy.info/)

- Você quer rodar Windows ou alguma distro de Linux. A opção que já vem com o Mac é usar o Bootcamp (que está em Utilities) para reparticionar seu HD e instalar um segundo sistema operacional. Porém, eu recomendo antes disso usar produtos como o open source [VirtualBox](http://www.virtualbox.org/wiki/Downloads), ou os comerciais [Parallels Desktop](http://www.parallels.com/download/desktop/) ou [VMWare Fusion](http://web.archive.org/web/20090719015503/http://www.vmware.com/products/fusion/). Todos são muito bons. Com ele você pode rodar o Windows XP ou o próximo Windows 7 (não recomendo tentar o Vista a menos que você tenha um Mac desktop com mais de 4Gb de RAM), Ubuntu, Fedora, etc. Na maior parte dos casos, até mesmo para rodar coisas pesadas como SQL Server com Visual Studio, funciona muito bem. Para jogos, não, melhor fazer o dual boot, ou melhor ainda, comprar um Playstation 3 ou XBox 360.
...
- O Mac precisa de muito pouca manutenção e a maioria das ferramentas já está embutida no próprio sistema operacional. Apenas como uma “casca” visual para acessá-los mais facilmente, use o [Onyx](http://web.archive.org/web/20090802092434/http://www.titanium.free.fr/pgs2/english/download.html) que provê um jeito de rodar todos os scripts de manutenção de uma só vez. Às vezes, quando o seu Mac começar a se comportar de forma estranha, rodar o Onyx costuma ajudar muito, embora essa situação seja muito rara.

- Como editor de textos, não tem jeito, meu preferido depois de 3 anos, continua sendo o TextMate. Para quem é mais purista de Linux talvez goste de usar o [MacVim](http://code.google.com/p/macvim/), especialmente com meu pacote de [snippets para Ruby](http://web.archive.org/web/20090725080422/http://akitaonrails.com/2009/04/27/the-best-environment-for-rails-on-windows-part-2) :-) Fora isso existem dezenas de outros editores, mas nenhum foi muito do meu gosto. Um plugin para deixar o Textmate ainda mais bonito é o [MissingDrawer](http://code.google.com/p/textmate-missingdrawer/), recomendo que tentem. Se você é desenvolvedor, siga meu repositório de [vimfiles](http://web.archive.org/web/20091005093524/http://github.com/akitaonrails/vimfiles) para ter seu Vim já pré-configurado com o que há de melhor.

- Para downloaders, o próprio Mozilla Firefox tem diversas extensions para isso, ou se você é um bom usuário Linux, saberá usar o bom e velho curl. Mas, se quiser algo mais visual, uma excelente alternativa é o comercial [Speed Download](http://yazsoft.com/), que tem várias opções para downloads paralelos, resumir downloads interrompidos e coisas do tipo. Ainda na sessão de downloads, se você gosta de Bit Torrent, um dos melhores clientes é o [Transmission](http://web.archive.org/web/20090719033742/http://www.transmissionbt.com/download.php)

- Para gravar DVDs, o próprio Mac já consegue fazer isso razoavelmente bem. Mas se quiser mais controle e detalhes, um dos mais tradicionais e melhores é o comercial [Toast](http://www.roxio.com/enu/products/toast/default.html) da Roxio, o equivalente do Nero no Windows. Para algo mais simples tem o [Disco](http://web.archive.org/web/20090721195744/http://www.discoapp.com/), um dos visualmente mais bonitos, onde um dos recursos é um banco de dados dos DVDs que você já gravou para facilitar procuras. E um freeware razoável é o [Burn](http://burn-osx.sourceforge.net/Pages/English/home.html).
