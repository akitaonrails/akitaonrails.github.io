...
Estimating tomorrow's weather as rainy does not MAKE it rain. Estimating the results of the Super Bowl does not MAKE it happen. Therefore, there is zero correlation between an estimate and the actual outcome.
...
The most precise level of detail a feature scope can have is the actual coding of the feature. This is important: the lines of code from programming are **NOT** the outcome. The execution of that code in runtime is what end users will actually experience.

The programming itself is the **ACTUAL blueprint**. The diagrams, the use cases, the user stories or any other non-programming before that is just a draft, a mere sketch.

A naive architect or designer might think that the detailed diagrams or use case documents, fancy powerpoint slides, have the same worth as an engineering blueprint. But they do not: they're the equivalent of a **sketch on a napkin**. Volatile, mostly worthless actually.
...
This is the metaphor that makes non-programmers crazy: in engineering, the construction itself is the most expensive and time-consuming part. In programming, the blueprinting (the coding) is the expensive part, and the "construction" is actually trivial (just compiling the code, which is automatic).

The same way, a project scope is just a set of sketches. We should dump the notion that there will ever exist a **"complete"** scope of a project. There is no way to say "100% of the scope" or "closed scope", because a software project scope is, by definition, always incomplete.

Moreover, I will argue that approximately 80% of this so called "scope" - what I prefer to call sketch - is mostly worthless to most end-user activities (the admin section, institutional pages that no one reads, complicated signing up processes, etc).

This is why every feature list MUST be prioritized. You usually can get by with 20% of the features (this is roughly what people mean when they say "MVP" or "minimum viable product"). **Release as early as you can,** get user feedback and refine the rest of the "sketch" you call backlog.

So, instead of aiming for an all-or-nothing proposition of having to find stupidly complex equations to figure out a "precise" estimate for an incomplete sketch, assume you can actually deliver EARLY, the first 20% that actually matters and figure out the rest in iterations.
...
Expose the small mistakes early, correct them often. Implementing everything in a black-box and doing a Big Bang deployment is the easiest path to **failure**. Delivering often, exposing the bugs and fixing them constantly is accepting that mistakes will happen and gaining strength in the process as you go.
...
The thing about those furnaces is that if you heat them too much, they can explode in your face. If you cool it down too much, the ore will harden.

Your job is to add coal to the furnace. You decide at what rate. If you go too fast it gets exploding hot. If you go too slow you may extinguish the fire and lose your furnace.
...
You're safe within a certain margin of temperatures and you speed up or slow down the coal intake by checking the thermometer all the time.
...
If the Velocity is too high, your team is probably working either extra time or delivering lower quality code. This will backfire because either your team will burn out too fast or the code is accumulating [technical debt](http://www.akitaonrails.com/en/2017/06/22/a-economia-do-desenvolvimento-de-software) too fast and you won't be able to pay back. Velocity will drop to a halt if you keep this high (furnace explodes).

If the Velocity is too low, your team is slacking off, your backlog is a useless piece of shit that no one can understand even after 10 hour meetings, or you already let the Velocity get too high and now you're paying back technical debt or your team is dead after burnout (fire extinguished).
...
Start the iterations. After each iteration release to a staging environment. Non-programmers, beware: ALWAYS make sure whichever programmer you hire ALWAYS delivers testable versions of the delivered stories to a publicly available URL that you can actually visit and test.
...
Pareto again: Agility is about Risk Management. You accept that losing 20% of your budget is ok. And you play with that. But that's also okay, because you only actually need a bit more than 20% of the sketchy scope you have.
...
Velocity is meant to be kept steady. Not always increasing. Not always volatile and unpredictable. Keep velocity in a predictable rate and use the variance as an indication of being too fast or too slow, adjust the other variable, measure again and go. Just like a furnace thermometer.
