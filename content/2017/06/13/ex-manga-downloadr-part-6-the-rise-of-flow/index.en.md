...
As a quick recap, the exercise is that I want to scrape MangaReader.net for a bunch of images, organized in pages and chapters, and in the end it should compile organized PDFs so I can load them on a Kindle device.

Web scraping is a simple loop of HTTP GETs over a ton of URLs, scraping the HTML, and fetching more URLs to download.

In many simple languages, people usually solve this naively in 2 ways:

* A simple nested loop. One single thread, sequential fetches. So if you have 5,000 links and each links takes 10 seconds to fetch, it's basically 10 * 5,000 = 50,000 seconds, which is a stupid long time.
* A simple spawn of processes, fibers, threads or parallel I/O in a reactor, all at once. An attempt to parallelize all fetches at once.

Everybody probably agrees that the first option is stupid. Now, the second one is tricky.
...
The message is: it's damn trivial to spawn parallel stuff. it's damn complicated to control parallel stuff.
...
Then, [Flow](https://github.com/elixir-lang/flow) is an easier high abstraction that you can use almost the same way you would use `Enum` in your collections, but instead of sequential mapping, it takes care of parallel traversing and control of batches. So the code is very similar but without you having to control the parallelization controls manually.
