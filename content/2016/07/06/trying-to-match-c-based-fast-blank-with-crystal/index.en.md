...
Sam Saffron, from Discourse, also built a very small gem called ["fast_blank"](https://github.com/SamSaffron/fast_blank) which is a tiny library written in C that reimplements ActiveSupport's `String#blank?` method to be up to 9x faster. Because Rails digests volumes of strings, checking if they are blank every time, this adds some performance (depends on your app, of course).
...
* Make the performance be faster than Ruby and close to C - Not so much yet (under OS X the performance is quite good, but under Ubuntu it doesn't scale so well for large strings)
...
Hell yeah! If you're a rubyist I bet you can understand 100% of the snippet above. It's not "exactly" the same thing (as the specs are not fully passing yet), but it's damn close.
...
Finally, every time you have a rubygem with a native extension, you will find this bit in their gemspec files:
...
This happens with all FFI-like extensions but it doesn't happen to the pure C implementation. In Sam Saffron's C implementation it directly works with the same address in C-Ruby's memory space:
...
It receives a pointer (direct memory address) and goes. And this is a huge advantage for the C version. When you have a big volume of medium to large sized strings being copied over from C-Ruby to Crystal, it adds a noticeable overhead that can't be removed.
...
The fast C version is called **"Fast Blank"** but although you can consider it "correct" it's not compatible with all the edge cases from Rails. So he implemented a `String#blank_as?` which is compatible with Rails. Sam calls it **"Fast Activesupport"**.
...
The benchmark keeps comparing against strings of larger and larger sizes, from 6, to 14, to 24, up to 136 characters in length.
...
It's also faster, but just by 2 to 3 times compared to pure Ruby, a far cry from 11x. But my hypothesis is that the mapping and copying of so many strings over adds a large overhead that the C version does not have.

And the **Crystal version over Ubuntu**:
...
Again, the Ubuntu versions of both the Crystal library and the Ruby binary run faster and the comparison shows no more than twice as much faster. And pure Ruby's `String#empty?` is in the same ballpark as Crystal's version.

### Conclusion

The most obvious conclusion is that I probably made a mistake in choosing Fast Blank as my first proof of concept. The algorithm is too trivial and a simple check for `String#empty?` in pure Ruby is orders of magnitude faster than the added overhead of mapping and string copying to Crystal.
...
That being said, I still believe that this exercise was well worth it. I will probably do some more. I'd really want to thank Ary (Crystal creator) and Paul Hoffer for the patience in helping me out through many of the quirks I found along the way.
...
I hope the code I published here will serve as boilerplate examples for more Crystal-based Ruby extensions in the future!
