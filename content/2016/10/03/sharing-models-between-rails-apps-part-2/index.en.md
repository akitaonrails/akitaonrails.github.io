...
Let's continue from where I left off in [Part 1](http://www.akitaonrails.com/en/2016/10/03/compartilhando-models-entre-aplicacoes-rails-parte-1) where I quickly described how you can extract reusable model logic from a Rails app into a testable Rubygem.
...
The `migration_template` will take care of adding the proper timestamp to the migration file, so you don't have to add it manually and the template file name can be something plain such as [`migrations/add_role_field_to_user.rb`](https://github.com/Codeminer42/cm42-central-support/blob/master/lib/generators/central/templates/migrations/add_role_field_to_users.rb).

All that having been said, there is a second challenge I added to my internal secondary app: I wanted it to have its own main database and use Central's database as a secondary read-only source.

So migrations in the secondary app (let's just call it Central-2) will run against its own main database, not against the main Central's database. This adds the following problem: the test suite must be able to create and migrate both test databases, in isolation from Central. Only in production should Central-2 connect to Central's database.
...
This is how I define a namespace of Rake tasks that start with `central:`, and every one of those connects to the secondary database as described in `database_central.yml`. The weird syntax is from my [`chainable_methods`](https://github.com/akitaonrails/chainable_methods), don't mind it too much.
...
Then we have a `default_scope` locking down the model as `readonly`. We don't want that in the test environment because I still want to have Factory Girl populate the test database with fake data for the specs. But it's a good idea to have it in the production environment just to make sure.
...
If you didn't already, refer to [Part 1](http://www.akitaonrails.com/en/2016/10/03/compartilhando-models-entre-aplicacoes-rails-parte-1) for more details.
...
In this particular case, the recommended approach is to create a [Follower database](https://devcenter.heroku.com/articles/heroku-postgres-follower-databases), which is how Heroku calls a secondary replicated database, and make my secondary application connect to that (since it only needs a read-only source).
