...
Yesterday [I posted](http://www.akitaonrails.com/en/2016/06/06/manga-downloadr-portando-de-crystal-para-ruby-e-um-pouco-de-jruby) how I built a new implementation of the Manga Reader Downloader in Crystal, ported to Ruby, tested on JRuby and compared against Elixir. Just to recap, these were the results to fetch chapters, pages and image links of a sample manga:
...
So, Elixir is still at least twice as fast as Crystal at this point. But this also demonstrates how a different algorithm does make a huge difference where it matters. I can probably tweak it a bit more but this should suffice for now.
...
The idea is that we will have a fixed number of spawned native threads (in this case, determined by `@config.download_batch_size`). As one thread finishes it will pop a new link from the `collection` Array, essentially working as a "queue" to be depleted.
...
And Crystal is the fastest, as you would expect because it's a super optimized binary doing CPU bound operations, clocking in at a bit more than 1 second.
