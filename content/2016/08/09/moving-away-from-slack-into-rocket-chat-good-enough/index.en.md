...
description: "I chose Rocket.chat to replace Slack and built a production setup with MongoDB in a replica set, multiple Node.js instances, NGINX, SSL, and monitoring. It works, but it’s not for beginners."
tags:
- reviews
- devops
- databases
...
In [my previous post](http://www.akitaonrails.com/en/2016/08/03/migrando-para-o-gitlab-sim-vale-a-pena) I forgot to mention minimal firewall and SSL configuration, but you can check it here.

One of the most difficult pieces to move away from is Slack. Don't get me wrong, it's a great tool and it gets the job done. The free-tier is really competent but I really want some of the paid features such as history of all conversations with search capabilities and uploads without limit.
...
Unfortunately, it has an important **show-stopper** for my needs. You can create both public channels or private groups, but anyone can delete a private group. If you want permission control for that you must use their ["Enterprise" offering](https://about.mattermost.com/pricing/) and pay **USD 20/year per user**.
...
If we see how the project is evolving over time, we can see that interest is diminishing. The lack of good automated tests also makes it more difficult for new people to contribute without adding regressions, and makes contribution reviews more difficult and time consuming.
...
The basic subscription for monitoring will cost you an extra USD 39 a month, but believe me when I say it's worth it to add all your MongoDB deployments under MMS.
...
And now you should have `https://yourdomain.com` already up and running. The first user that signs up will be the site administrator and from there you can customize Rocket.chat internally.
