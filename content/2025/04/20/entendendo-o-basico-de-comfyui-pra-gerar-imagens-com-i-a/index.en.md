...
More than that, ComfyUI also supports Nodes with models like Tencent's HunyuanVideo, which I introduced in another article from yesterday, about [FramePack](https://www.akitaonrails.com/2025/04/19/gerando-videos-de-ate-2-min-a-partir-de-uma-foto-com-i-a). In fact, we can do the same thing FramePack does but from a ComfyUI workflow: generate a video from an image, using the same model. FramePack is infinitely easier to use, but ComfyUI is the Swiss Army knife of AI: it does **EVERYTHING**, which is also why it's infinitely more complicated and you need to put in serious effort to learn how to use it.
