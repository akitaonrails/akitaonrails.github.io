...
- **RTC** (*Real-Time Clock*) is the microcontroller's real-time clock. It keeps running even when the device is "off" because it has its own coin-cell battery. Internally it stores time in registers such as `RTC->TR` and `RTC->SSR`. For example, `TR` might be `0x00000042` (counting seconds since some reference point) and `SSR` might be `0x0000FFFF` (sub-seconds). These values change every instant, but at the exact moment of boot they fall within a small, predictable window.
- **SysTick** is another timer, this one inside the ARM core itself, counting at high speed — typically from 0 to `0x00FFFFFF` and rolling over. When the firmware reads this counter at seed-creation time, it captures a value like `0x003D7A12`. The problem is that, if a stable RTC on cold boot restricts the boot moment to a few seconds, SysTick can only have started from a limited set of initial values — which is why [Block](https://engineering.block.xyz/blog/predictable-rng-fallback-and-32-bit-reseed-in-coldcard-firmware) estimates that alone can shrink the space to ~80,000 possibilities.
...
- If the RTC is stable on cold boot, as [Block](https://engineering.block.xyz/blog/predictable-rng-fallback-and-32-bit-reseed-in-coldcard-firmware) suggests, SysTick alone shrinks the space to ~80,000 values (~16 bits), making the search trivial in seconds.
