...
Maybe Fable is far better at other tasks. Its marketing is all about security analysis capability, and that capability is literally blocked in the public version (when it isn't busy blocking "hello"). And here's a speculation I can't help myself on. Anthropic itself documented that tasks deemed "high risk" get redirected to Opus 4.8. With a result this close to Opus, the itch won't go away: are they redirecting too much? How much of my benchmark was answered by the actual Fable, and how much fell back? From the outside, there's no way to audit it. Maybe Fable honestly ties with Opus at coding. Maybe I measured Opus wearing a Fable name tag. Both hypotheses produce exactly the same number on the table. On what can be measured here, it's a more expensive Opus.
...
And the practical recommendation, for the time being at least: **stay on Opus 4.8**. Fable delivers the same measurable quality at 10× the price, and carries the extra uncertainty of how much of each answer is even Fable. Beyond that, it stays identical to the [previous posts](/en/2026/06/01/llm-benchmarks-grok-4-3-minimax-m3-opus-4-8/): for programming, by default, Opus and/or GPT (I use both simultaneously). So far, the only thing Fable provably does better than Opus is marketing.
