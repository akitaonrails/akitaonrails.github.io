...
Isso funcionou bem o bastante pra fechar um batch inteiro, mas depois de escutar com calma ficou claro que ainda tinha um problema chato: legenda não é roteiro falado. Legenda é otimizada pra tela. Ela quebra um parágrafo longo em várias cues pequenas pra caber em duas linhas, acompanhar o olho do espectador e não cobrir metade do vídeo. Só que na hora de montar áudio, cada cue pequena vira uma janelinha de tempo. O TTS termina uma frase, sobra meio segundo aqui, 300 ms ali, mais um respiro artificial no meio de uma ideia. A duração total bate com o vídeo, mas o resultado soa picotado. Parece alguém lendo legenda, não alguém explicando.

O conserto foi voltar pro roteiro original dos posts do blog. Quase todo episódio do Akitando tem um post correspondente com a seção `## Script`, que é o texto que eu lia de verdade quando gravei o vídeo. O novo chunker descobre qual post corresponde ao `video_id`, extrai os parágrafos em português, mapeia cada parágrafo pros ranges de cues do `.pt-BR.srt` por substring, e usa esse mapa pra agrupar o `.en.srt` em chunks que seguem os parágrafos originais. Se uma cue fica atravessada entre dois parágrafos, ele corta perto da pontuação mais próxima e divide o timestamp proporcionalmente. Se um parágrafo passa de 700 caracteres, aí sim ele subdivide em fim de frase.

Foi aí que o áudio começou a soar natural. Em vez de 19 blocos grandes com pausas estranhas no meio, o ep001 virou 32 chunks, praticamente um por parágrafo, com só 16 segundos de silêncio morto no episódio inteiro. O alvo de stretch também mudou: antes eu tentava preencher 92% da janela; agora dá pra mirar em 95%, porque a janela já representa um parágrafo falado de verdade, não um pedaço arbitrário de legenda de tela.
...
A parte que dói no bolso é exatamente essa mudança de chunking por parágrafo. Como as fronteiras dos chunks mudaram, o cache antigo deixou de servir. Mesmo que o texto traduzido seja quase o mesmo, o áudio precisa ser gerado de novo porque agora cada chamada ao TTS tem outro começo, outro fim, outro contexto e outra prosódia. Então, sim: estou rodando uma segunda geração completa dos 146 episódios. Vai queimar crédito de novo. Mas é o tipo de gasto que muda o resultado final de "tecnicamente alinhado" pra "dá pra ouvir uma hora sem sentir que tem um robô tropeçando em pausas de legenda".
...
O outro problema é o tempo. Cada episódio leva uns 20 minutos pra rodar, mesmo paralelizando várias chamadas (aumentei o paralelismo depois que subi pra Business, que permite mais). O primeiro batch inteiro levou pouco menos de dois dias pra completar. Resultado daquela rodada: **146 de 146 episódios dentro de ±1 segundo da duração do YouTube original**. Desses, 144 passaram automaticamente pelo length-fit da pipeline sem intervenção nenhuma. Três (ep006, ep009, ep020) bateram no teto antigo de 1.05× do `atempo` e tiveram que ser ajustados manualmente pro comprimento exato do vídeo — subi o `MAX_ATEMPO_RATIO` pra 1.08 depois disso pra que rodadas futuras não esbarrem no mesmo limite. Zero chunks falharam no batch inteiro: nenhum retry por alucinação, nenhum áudio rejeitado pelo sanity check.

Só que "dentro de ±1 segundo" mede duração, não naturalidade. Antes de subir tudo pro YouTube, veio essa percepção das pausas artificiais criadas por cues pequenas demais. Traduzindo: a primeira rodada provou que a pipeline fecha 146 episódios ponta a ponta; a segunda rodada, com chunking por parágrafo, está regenerando todos os áudios pra deixar a fala mais contínua. É mais caro e chato, mas ainda é melhor descobrir isso antes do upload do que depois de publicar 96 horas de dublagem com respiro esquisito no meio de frase.

Depois dessa segunda rodada, falta subir os arquivos de áudio em inglês pra cada vídeo, aproveitando o suporte do YouTube pra [múltiplas trilhas de áudio por vídeo](https://support.google.com/youtube/answer/13338784).
