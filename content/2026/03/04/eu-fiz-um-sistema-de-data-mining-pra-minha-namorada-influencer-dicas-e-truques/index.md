...
date: '2026-03-04T18:00:00-03:00'
tags:
  - Ruby on Rails
  - Data Mining
  - LLM
...
> "Maior dificuldade hoje, é saber qual tipo de conteúdo criar que pode dar muito certo e porque deram certo, o que fez dar o resultado que deu. Tem que ler comentários dos videos pra entender motivos gerais."
...
Ela não pediu um dashboard. Não pediu gráficos. Não pediu relatórios em PDF. Pediu respostas pra problemas concretos do dia a dia dela. Isso mudou completamente como eu abordei a arquitetura.
...
Esses números não são mérito meu sozinho. Claude fez boa parte do código. Mas a direção, as decisões arquiteturais, e principalmente a validação de cada step foram humanas. Claude não sabe o que uma influencer brasileira de cosplay precisa. Eu também não sabia — mas ela me disse.
...
## De Kamal pra Docker Compose: Saber Quando Pivotar

O IDEA.md original dizia "deploy com Kamal". Faz sentido: Kamal é a ferramenta de deploy do Rails, zero-downtime, rolling deploys. Mas na prática, o bot do Discord precisa ficar rodando como um processo separado, o Solid Queue é outro processo, e o Chrome headless é mais um. São 4 processos com ciclos de vida diferentes.

Kamal funciona bem pra deploy de um web app com workers. Mas pra 4 serviços com dependências de saúde entre eles (o Discord e os jobs só sobem quando o app retornar 200 no health check), Docker Compose com `condition: service_healthy` foi a escolha certa. Deploy é `docker compose pull && docker compose up -d`. Sem orquestrador, sem complexidade extra.

Essa decisão não estava no plano original. Apareceu quando eu tentei configurar o Kamal e percebi que ia lutar contra a ferramenta ao invés de usá-la.
...
Esse tipo de coisa não aparece em nenhum tutorial. Você descobre depois de 2 horas de `connection refused` e leitura do código fonte do `chromedp/headless-shell`.
...
- `camila_profiles.yml` — usernames nas plataformas
...
Depois agrupa tudo em entidades `Creator`. A influencer é profundidade 0. Concorrentes diretos, profundidade 1. Menções de concorrentes, profundidade 2. Limite máximo de 3 pra evitar cascata infinita.

O depth limiting foi uma decisão que apareceu depois de um teste onde o pipeline promoveu 15 perfis de uma vez e o custo de API triplicou. Sem o limite, o sistema expandia exponencialmente a cada ciclo.

## De Relatório Semanal pra Digests Diários: Ouvindo a Usuária

O plano original era um relatório semanal por email com 9 seções. Implementei, rodou, mandei pra ela. O feedback foi honesto: "é muita informação de uma vez, eu leio na segunda e na quarta já esqueci o que tinha lido".
...
Também rastreia lançamentos de filmes/séries via TMDB, jogos via IGDB (com OAuth do Twitch), animes via AniList — tudo filtrado pra conteúdo geek, janela de 90 dias pra frente. Aniversários de franchises (usando TMDB, IGDB, AniList, Wikimedia) e feriados brasileiros com cálculo de datas móveis. Sim, tem um calculador de Páscoa no código baseado no algoritmo computus. E notícias via scraping de RSS e X/Twitter a cada 6 horas, classificadas por relevância ao nicho.
...
Decisão controversa que eu não pretendo defender extensivamente: SQLite3 em produção. Com WAL mode, um único writer por vez é suficiente porque o sistema tem um padrão previsível de escrita (jobs sequenciais, nunca concorrentes no mesmo segundo). Os dados ficam num bind mount do Docker (`./data/storage:/rails/storage`) e backup é `cp data/storage/*.db backups/`.
...
O Instagram bloqueia scraping aleatoriamente. Não importa quão stealth seu browser é. A configuração inicial era só scraping via Ferrum. Durou 3 dias antes do primeiro bloqueio. Adicionei o Apify como fonte primária (API paga, mais confiável) e o scraping virou fallback.
...
Os tool calls falham em parâmetros com frequência que me surpreendeu. O LLM pede `limit: 100` quando o máximo é 50, ou manda o username sem o @ quando deveria ter. Clamping silencioso (`[[val.to_i, 1].max, 50].min`) e normalização de input (`.strip`, `.delete("@")`) em toda tool eliminaram uma categoria inteira de erros que estavam consumindo tokens em loops de retry.
...
## O Gemfile: 14 Gems Que Resolvem Tudo
...
11 gems de produção. Sem Redis. Sem Sidekiq. Sem Devise (não tem login). Sem ActiveAdmin (não tem admin). Sem Webpacker/esbuild (não tem frontend). O Gemfile mais enxuto de qualquer projeto Rails que eu fiz.

Solid Queue no lugar de Sidekiq eliminou Redis do stack. Solid Cache dispensou Memcached. SQLite dispensou Postgres. Cada dependência que sai do projeto é uma classe inteira de problemas de produção que desaparece junto.
...
Se eu tivesse começado pela arquitetura, teria passado semanas projetando um dashboard que ela não ia abrir. Começando pelo desejo, o sistema acabou sendo um chatbot no Discord que ela usa todo dia. Jobs assíncronos que não são idempotentes são bombas-relógio — o rate limit vai estourar, o scraping vai falhar, o container vai reiniciar, e se o job não sobrevive a isso sem duplicar dados, o sistema não funciona. E o feedback real da usuária mudou o projeto mais que qualquer decisão técnica minha: relatório semanal virou digest diário, Kamal virou Docker Compose, scraping direto virou Apify com fallback. Nenhuma dessas mudanças estava no plano.
